...
//**********************
// taken from git@github.com:wknechtel/3d-bin-pack.git, slightly updated to be runable
#include <time.h>
#include <stdio.h>
#include <string.h>
#include <stdlib.h>
...
{ 
  ...
  TIMELIMIT_SECONDS = argc >= 3 ? atoi(argv[2]) : -1;
...
void execiterations(void) 
{
  for (variant = 1; (variant <= 6) && !quit; variant++)
  {
    ...
    for (layersindex = 1; (layersindex <= layerlistlen) && !quit; layersindex++)
    {
      ...
      if (TIMELIMIT_SECONDS != -1 && elapsedtime > TIMELIMIT_SECONDS) {
        printf("\ntimelimit reached (%d s). abort\n", TIMELIMIT_SECONDS);
        return;
      }
      packedvolume = 0.0;
      packedy = 0;
